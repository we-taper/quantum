...
void CreateGradientCircuit(
  ...
  qsim::BasicGateFuser<qsim::IO, QsimGate>::Parameter param;
  ...
  for (int i = 0; i < grad_gates->size(); i++) {
    ...
    (*partial_fuses)[i] = fuser.FuseGates(param, circuit.num_qubits, left, right);
    left = right + 1;
  }
  right = circuit.gates.end();
  (*partial_fuses)[grad_gates->size()] =
      fuser.FuseGates(param, circuit.num_qubits, left, right);
}
